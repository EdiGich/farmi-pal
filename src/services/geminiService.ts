...
export async function chatWithFarmiPal(message: string, history: { role: 'user' | 'model', parts: { text: string }[] }[] = []) {
  try {
    ...
    return response.text || "Pole sana, nimevamiwa na glitch kidogo. Hebu jaribu tena.";
  } catch (error) {
